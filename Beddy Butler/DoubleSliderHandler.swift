...
class DoubleSliderHandler: NSView {
    ...
    var sliderBarImage: NSImage {
        ...
        let rect = NSMakeRect(0, 0, barImage.size.width, barImage.size.height)
    ...
    func addHandle(name: String, image: NSImage, iniRatio: CGFloat,
                   sliderValue: @escaping SliderValue,
                   sliderValueChanged: @escaping SliderValue) {
        ...
        for view in self.subviews {
            if view.tag == dynamicViewTag {
                if NSPointInRect(mouseDownPoint, view.frame) {
                    ...
                    self.activeBoundary = self.boundaryForView(view: self.activeHandleView!)
    ...
    override func mouseDragged(with theEvent: NSEvent) {
        ...
        if Float(difference) > Float.ulpOfOne && self.sliderValueChanged != nil {
            self.lastValue = self.activeHandle?.curValue
            ...
            for key in self.handles {
                ...
                if Float(abs(curValue - oldValue!)) > Float.ulpOfOne {
                    // VALUE CHANGED
    ...
    func updateValues(notification: NSNotification) {
        if let updateObject = notification.object as? (String, CGFloat) {
            ...
            var ratio = handle?.ratioForValue(value: newValue)
            let slidableWidth = self.bounds.size.width - self.sliderHandleWidth
            let boundary = self.boundaryForView(view: (handle?.handleView)!)
            ...
            if Float(abs(ratio! - (handle?.curValue)!)) > Float.ulpOfOne {
                handle?.curValue = ratio!
                let newX = slidableWidth * ratio!
                handle?.handleView.frame = NSMakeRect(newX - handle!.handleView.frame.size.width * 0.5,
                                                      handle!.handleView.frame.origin.y,
                                                      handle!.handleView.frame.size.width, handle!.handleView.frame.size.height)
